...
def build_flashcards_apkg(
    ...
    """
    try:
        # Hierarquia "<root>::<ano>::<UC>::<aula>" (config.yaml > app:
        # anki_root_deck + anki_ano_deck, padrão "Medicina::Segundo Ano") - tem
        # que bater com a MESMA hierarquia usada por core/anki_connect.py (sync
        # ao vivo via AnkiConnect), senão o .apkg (Drive, sempre gerado) e o
        # Anki ao vivo (só quando disponível) divergem em qual deck a aula cai.
        deck_id = _stable_id(unit_code, lesson_name)
        root_deck = settings.app.anki_root_deck
        ano_deck = settings.app.anki_ano_deck
        deck_name = f"{root_deck}::{ano_deck}::{unit_code}::{lesson_name}"
