...
# Templates/CSS copiados verbatim do Anki real (modelTemplates/modelStyling via
# AnkiConnect) - byte a byte iguais ao que já existe, pra importar sem o Anki
# achar que o note type mudou de forma incompatível.
...
def _mc_note(card: Dict[str, Any], materia: str) -> genanki.Note:
    ...
    fields = [
        card.get("enunciado", ""),
        materia,
        "",  # Imagem - extração de imagem de slide não implementada ainda
        "",  # Imagem_Verso - idem
...
def _vf_note(card: Dict[str, Any], materia: str) -> genanki.Note:
    fields = [
        card.get("assertiva", ""),
        materia,
...
def build_flashcards_apkg(
    ...
    try:
        ...
        for card in flashcards:
            ...
            if tipo == "mc":
                deck.add_note(_mc_note(card, unit_code))
                count_mc += 1
            elif tipo == "vf":
                deck.add_note(_vf_note(card, unit_code))
                count_vf += 1
            else:
        ...
        logger.info(f".apkg gerado com {count_mc} MC + {count_vf} VF em: {output_path}")
